      ...
      child: isLoading
        ...
        onPressed: () async {
          if (controller.text.isEmpty) {
            ...
            onVerify();
            try {
              await Provider.of<AuthProvider>(context, listen: false).verifyOTP(controller.text);
...
class ResendOTP extends StatelessWidget {
  ...
  const ResendOTP({Key? key, required this.resendSeconds, required this.onResend}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        GestureDetector(
          onTap: resendSeconds > 0 ? null : () async {
            try {
              String phoneNumber = Provider.of<AuthProvider>(context, listen: false).mobile ?? "";
              await Provider.of<AuthProvider>(context, listen: false).login(phoneNumber);
          ...
          child: Text(
            ...
            style: TextStyle(
              color: resendSeconds > 0 ? Colors.grey : Theme.of(context).primaryColor,
